...
func Test_configuration_verifyConfig(t *testing.T) {
	tests := []struct {
		...
		{"DropSchema and UseExistingSchema both set to true", configuration{Hostname: "localhost", Port: "30015", Username: "Sausage", Password: "QuietBadPassword", Schema: "GS", DropSchema: true, Workers: 10, Customers: 1000, Orders: 1000000, TrnxRecords: 50000000, StartYear: 2010, EndYear: 2015, UseExistingSchema: true}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
